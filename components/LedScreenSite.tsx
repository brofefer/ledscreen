...
export default function LedScreenSite() {
  ...
  return <main>
    <header className="site-header"><a className="brand" href="#inicio"><BrandLogo /><span>LedScreen</span></a><nav className="desktop-nav"><a href="#inicio">Inicio</a><a href="#servicios">Servicios</a><a href="#pantallas">Pantallas LED</a><a href="#eventos">Eventos</a><a href="#trabajos">Trabajos</a><a className="nav-accent" href="#cotizador">Cotizá</a><a href="#faq">FAQ</a><button className="lang-button" onClick={() => setLang(lang === "ES" ? "EN" : "ES")}>{lang === "ES" ? "EN" : "ES"}</button><a className="nav-cta" href="#contacto">Cotizar</a></nav><button className="menu-button" aria-label="Abrir menú" aria-expanded={menu} onClick={() => setMenu(!menu)}><i /><i /><i /></button></header>
    {menu && <div className="mobile-menu"><a onClick={() => setMenu(false)} href="#inicio">Inicio</a><a onClick={() => setMenu(false)} href="#servicios">Servicios</a><a onClick={() => setMenu(false)} href="#pantallas">Pantallas LED</a><a onClick={() => setMenu(false)} href="#eventos">Eventos</a><a onClick={() => setMenu(false)} href="#trabajos">Trabajos</a><a onClick={() => setMenu(false)} href="#cotizador">Cotizador 3D</a><a onClick={() => setMenu(false)} href="#faq">FAQ</a><div className="mobile-socials">{SOCIALS.map((item) => <a aria-label={item.label} href={item.href} key={item.label}>{item.icon}</a>)}</div></div>}
