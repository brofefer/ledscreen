...
export default function SceneObject({ id, label, defaultPosition, defaultRotationY = 0, transform, selected, moveMode, bounds, forbidden, axis = "xz", canRemove = true, size, onSelect, onChange, onToggleMove, onReset, onRemove, children }: Props) {
  ...
  return <group position={position} rotation={[0, rotationY, 0]} onClick={(event) => { event.stopPropagation(); if (!moveMode) onSelect(id, label, { position, rotationY }); }} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp}>
    ...
    {selected && <Html position={[0, size[1] + .28, 0]} center distanceFactor={9} zIndexRange={[80, 40]}><div className="object-popover" onPointerDown={(event) => event.stopPropagation()}><div className="object-popover-title"><span>{label}</span><small>{moveMode ? "Deslizá para mover" : "Seleccionado"}</small></div><div className="object-popover-actions"><button className={moveMode ? "active" : ""} onClick={() => onToggleMove(id)}>{moveMode ? "Listo" : "Mover"}</button><button onClick={() => onChange(id, { position, rotationY: rotationY + Math.PI / 8 })}>Rotar</button><button onClick={() => onReset(id)}>Restablecer</button>{canRemove && <button className="danger" onClick={() => onRemove(id)}>Eliminar</button>}</div></div></Html>}
  </group>;
}
